...
// i want to make the torso dedocahedon
//

function torso() {
  let shape = createDeca();
  ...
  pointsArray = shape.positions;
  createBuffer();
  gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(instanceMatrix));
  // for (var i = 0; i < 2000; i++) gl.drawArrays(gl.TRIANGLE_FAN, 4 * i, 4);
...
function rightLowerArm() {
  instanceMatrix = mult(
    modelViewMatrix,
    translate(0.0, 1.0 * lowerArmHeight, 0.0),
  );
  instanceMatrix = mult(
    instanceMatrix,
    scale(lowerArmWidth, lowerArmHeight, lowerArmWidth),
...
function leftUpperLeg() {
  instanceMatrix = mult(
    modelViewMatrix,
    translate(1.2, 0.5 * upperLegHeight, 0.0),
  );
  instanceMatrix = mult(
    instanceMatrix,
...
function leftLowerLeg() {
  instanceMatrix = mult(
    modelViewMatrix,
    translate(1.2, 0.5 * lowerLegHeight, 0.0),
  );
  instanceMatrix = mult(
    instanceMatrix,
...
function init() {
  ...
  projectionMatrix = ortho(-9.0, 10.0, -10.0, 10.0, -10.0, 10.0);
...
// berarti ini harus kayak buat kayak pergerakan dari pada objectnya kaya gimana
// gerakannya  sudut gerakannya
